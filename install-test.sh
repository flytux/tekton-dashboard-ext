...
for (( c=0; c<$node_number; c++ )); do
    ...
    read -p "- 역할은 무엇인가요?  master, master-member, worker " node_role
...
# terraform node 설정 추가 kubeadm/01-variables.tf

for (( c=0; c<$node_number; c++ )); do
   echo "    ${node_hostnames[$c]} = { role = \"${node_roles[$c]}\", ip = \"${node_ips[$c]}\" }," >> kubeadm/01-variables.tf
done
